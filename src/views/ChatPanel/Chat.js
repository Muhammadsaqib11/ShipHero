...
export class Chat extends Component {
    ...
    constructor(props) {
	  ...
	  scrollToBottom = () => {
		...
		this.chatContainer.current.scrollIntoView({ bottom:0, behavior: 'smooth' })
	  }

	  getMesages = () => {
		axios.get("https://cleangreen.gov.pk/api/load-latest-messages" , config)

        .then((response) => {
		  
		  this.setState({fromMessage:response.data.messages})
		  console.log("fromMessage",this.state.fromMessage)
		  this.setState({currentUser:this.state.fromMessage[0].from_user.email})
		//   const currentUser=this.state.fromMessage[0].from_user.email;
      ...
      componentDidMount() {
        ...
        const pusher = new Pusher('4cdc41e6ca0976f5d5aa', {
          cluster: 'ap1',
          encrypted: true
        });
      ...
      handleTextChange(e) {
        ...
        axios.post("https://cleangreen.gov.pk/api/send-message", payload)

        .then((response) => {
		  console.log("response", response)
		  this.setState({text:response.data})
		  console.log("from1", this.state.fromMessage)
		  this.state.fromMessage.push(this.state.text)
		  console.log("from2",this.state.fromMessage)
			this.getMesages()
    ...
    <div className="container-fluid h-100">
			<div className="row justify-content-center" style={{height:'96vh'}}>
			
				<div className="col-md-8 col-xl-8 col-lg-12 col-md-8 col-sm-12 chat h-100">
					<div className="chatCard">
						...
						<div className="chatmainContainter"  id="chatid">
						<FromUserMessage />

						<ToUserMessage/>
						
						{/* {
								this.state.fromMessage.map(function(item, i){
								
									if(item.from_user.id === userInfo.id) 
										return <FromUserMessage data={item} />;
									return <ToUserMessage data={item}/>;
							})
							} */}
